...
// サマービーチ ゴールデンアワー 水面カラー定義
const vec3 COLOR_RIPPLE = vec3(1.0, 0.75, 0.45); // 夕日を反射したアンバー（琥珀色）
const vec3 COLOR_CORE   = vec3(1.0, 0.9, 0.7);   // タッチ直後の明るいピーチ・オレンジ
...
void main() {
    ...
    // 水面の環境光（夕日）の反射として、エッジやハイライトが優しく光るようにする
    // 発光しすぎず、背景色を透過させるためにアルファを抑えめにブレンド
    finalAlpha *= fadeOut * 0.6; // 透明感を保ちつつ、光の反射だけを乗せる
